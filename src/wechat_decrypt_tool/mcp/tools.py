...
def _member_overview(args: dict[str, Any], _: McpToolContext) -> dict[str, Any]:
    ...
    result = build_member_overview(
        ...
        main_thread=_str(args, "main_thread") or "rule",
    )
...
def _install_tools() -> None:
    ...
    _register("wechat.analytics.member_speaking_overview", "Return per-member speaking stats for one chat session: wxid, display name, message count, share, top topics, and a main thread (rule-based by default; set main_thread='llm' to generate with the configured LLM endpoint, which sends truncated recent samples to that endpoint).", object_schema({**COMMON_ACCOUNT, "username": string_schema("Session username. Group chats end with @chatroom."), "start_time": int_schema("Optional Unix seconds start.", minimum=0), "end_time": int_schema("Optional Unix seconds end.", minimum=0), "topics": int_schema("Topic keyword count per member.", minimum=0, maximum=30), "limit": int_schema("Maximum members returned.", minimum=1, maximum=500), "include_hidden": bool_schema("Include hidden sessions.", default=False), "include_official": bool_schema("Include official sessions.", default=False), "refresh": bool_schema("Ignore cache and recompute.", default=False), "main_thread": string_schema("'rule' (default, local) or 'llm' (requires WECHAT_TOOL_LLM_API_KEY).")}, required=["username"]), _member_overview, package="wechat.analytics")
